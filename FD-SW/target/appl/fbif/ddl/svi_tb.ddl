...
METHOD do_enable_position_limits
{
    LABEL "|en|Enable Position Limits";
    HELP HLP5("|en|Enable/Disable Position Limits");
    CLASS INPUT;
    DEFINITION
    {
        ...
        mb = MEMBER_ID(ACTUAL);
        READ_PARAM(id, mb, "MODE_BLK")
            status = get_unsigned_value(id, mb, &us8_buf);
        if ( !(us8_buf == _OUT_OF_SERVICE || us8_buf == _MANUAL) ) {
            get_acknowledgement("|en|MODE_BLK(Actual) is not in OOS or MAN mode.\n"
                                "Please set to the OOS or MAN mode.\n",dummy,dummy,0);
            METHOD_ABORT_MACRO
        }
        status = select_from_menu("|en|Select functions:\n", dummy, dummy, 0, \
                                  "|en|Set position limits;" \
                                  "Set position cut-off;" \
                                  "Disenable the both\n", &ivalue);
        display_message("|en|\n", dummy, dummy, 0);
        switch (ivalue){
        case 1:
            us8_hi = 0;
            us8_lo = 0;
            status = get_unsigned_value(ITEM_ID(PARAM.FINAL_VALUE_CUTOFF_HI), \
                                        MEMBER_ID(ENABLE), &us8_hi);
            status = get_unsigned_value(ITEM_ID(PARAM.FINAL_VALUE_CUTOFF_LO), \
                                        MEMBER_ID(ENABLE), &us8_lo);
            if ( us8_hi >0 || us8_lo >0 ) {
                status = put_unsigned_value(ITEM_ID(PARAM.FINAL_VALUE_CUTOFF_HI), \
                                            MEMBER_ID(ENABLE), 0);
                status = put_unsigned_value(ITEM_ID(PARAM.FINAL_VALUE_CUTOFF_LO), \
                                            MEMBER_ID(ENABLE), 0);
                display_message("|en|The position cut-offs are disenabled.", dummy, dummy, 0);
            }
            EDIT_ENUM_MACRO(POSITION_LIMITS, ENABLE_HI, "Enable Hi");
            status = get_unsigned_value(id, mb, &us8_buf);
            if ( us8_buf == 1) {
                EDIT_FLOAT_MACRO_SUB(POSITION_LIMITS, LIMIT_HI, "Limit HI", "%%");
            }
            EDIT_ENUM_MACRO(POSITION_LIMITS, ENABLE_LO, "Enable Lo");
            if ( us8_buf == 1) {
                EDIT_FLOAT_MACRO_SUB(POSITION_LIMITS, LIMIT_LO, "Limit LO", "%%");
            }
            EDIT_ENUM_MACRO(POSITION_LIMITS, ENABLE_RATE_HI, "Enable Rate Hi");
            status = get_unsigned_value(id, mb, &us8_hi);
            EDIT_ENUM_MACRO(POSITION_LIMITS, ENABLE_RATE_LO, "Enable Rate Lo");
            status = get_unsigned_value(id, mb, &us8_lo);
            if ( us8_hi!=0 || us8_lo != 0 ) {
                EDIT_FLOAT_MACRO_SUB(POSITION_LIMITS, LIMIT_RATE, "Limit Rate", "%%");
            }
            break;
        case 2:
            us8_hi = 0;
            us8_lo = 0;
            status = get_unsigned_value(ITEM_ID(PARAM.POSITION_LIMITS), \
                                        MEMBER_ID(ENABLE_HI), \
                                        &us8_hi);
            status = get_unsigned_value(ITEM_ID(PARAM.POSITION_LIMITS), \
                                        MEMBER_ID(ENABLE_LO), \
                                        &us8_lo);
            if ( us8_hi != 0 || us8_lo != 0 ) {
                status = put_unsigned_value(ITEM_ID(PARAM.POSITION_LIMITS), \
                                            MEMBER_ID(ENABLE_HI), 0);
                status = put_unsigned_value(ITEM_ID(PARAM.POSITION_LIMITS), \
                                            MEMBER_ID(ENABLE_LO), 0);
                SEND_PARAM(ITEM_ID(PARAM.POSITION_LIMITS), MEMBER_ID(ENABLE_HI), "POSITION_LIMITS.ENABLE_HI");
                SEND_PARAM(ITEM_ID(PARAM.POSITION_LIMITS), MEMBER_ID(ENABLE_LO), "POSITION_LIMITS.ENABLE_LO");
                display_message("|en|The position limits are disenabled.", dummy, dummy, 0);
            }

            EDIT_ENUM_MACRO(FINAL_VALUE_CUTOFF_HI, ENABLE, "CUT-OFF HI");
            if ( us8_buf == 1) {
                /* example value is 98-100, other value will be range error. */
                EDIT_FLOAT_MACRO_SUB(FINAL_VALUE_CUTOFF_HI, CUTOFF_POINT_HI, "CUTOFF HO", "%%")
                    }

            EDIT_ENUM_MACRO(FINAL_VALUE_CUTOFF_LO, ENABLE, "CUT-OFF LO")
                if ( us8_buf == 1) {
                    EDIT_FLOAT_MACRO_SUB(FINAL_VALUE_CUTOFF_LO, CUTOFF_POINT_LO, "CUT-OFF LO", "%%")
                        }
            break;
        case 3:
            id = ITEM_ID(PARAM.POSITION_LIMITS);
            mb = MEMBER_ID(ENABLE_HI);
            status = put_unsigned_value(id, mb, 0);
            SEND_PARAM(id, mb, "POSITION_LIMITS.ENABLE_HI");

            mb = MEMBER_ID(ENABLE_LO);
            status = put_unsigned_value(id, mb, 0);
            SEND_PARAM(id, mb, "POSITION_LIMITS.ENABLE_LO");

            id = ITEM_ID(PARAM.FINAL_VALUE_CUTOFF_HI);
            mb = MEMBER_ID(ENABLE);
            status = put_unsigned_value(id, mb, 0);
            status = SEND_PARAM(id, mb, "FINAL_VALUE_CUTOFF_HI.ENABLE");

            id = ITEM_ID(PARAM.FINAL_VALUE_CUTOFF_LO);
            mb = MEMBER_ID(ENABLE);
            status = put_unsigned_value(id, mb, 0);
            status = SEND_PARAM(id, mb, "FINAL_VALUE_CUTOFF_LO.ENABLE");
            break;
        }
        METHOD_FINISHED_MACRO
   }
...
METHOD  do_manual_hi_stop
{
    LABEL "|en|Manual Find HI Stops";
    HELP HLP5("|en|Manual High Stop Limit adjustment");
    CLASS OUTPUT;
    DEFINITION
    {
        COMMON_LOCAL_VARS;
        ...
        get_acknowledgement("Run manual stop limits method\n", ids, dummy, 0);

    }
}

METHOD  do_manual_lo_stop
{
    LABEL "|en|Manual Find LO Stop";
    HELP HLP5("|en|Manual How Stop Limit adjustment");
    CLASS OUTPUT;
    DEFINITION
    {
        COMMON_LOCAL_VARS;
        ...
        CHECK_DEVICE_IN_PROCESS;
#if 0
Should ask to put the block in MAN mode if necesary
Should ask for Open stop Adjustment
Should allow to set the Travel and Travel Units
Should Run Auto Find Stops
Should set the Travel Calibration parameter

#endif        
        get_acknowledgement("will be implemented\n", ids, dummy, 0);

    }
}

METHOD  do_open_stop_adjustment
{
    LABEL "|en|Open Stop Adjustment";
    HELP HLP5("|en|Open Stop Adjustment");
    CLASS OUTPUT;
    DEFINITION
    {
        unsigned long   id;
...
METHOD  do_full_open
{
    ...
    DEFINITION
    {
        ...
        /* start FIND_SETOPEN */
        id = ITEM_ID(PARAM.FIND_STOPS);
        status = put_unsigned_value(id, 0, FINDSTOPS_SETOPEN);
        SEND_PARAM(id, 0, "FIND_STOPS");
        /* read status of running find stops procedure */
        chk_cnt = 0;
        do {
            /* check the status for find stops in the device status*/
            id = ITEM_ID(PARAM.FIND_STOPS);
            READ_PARAM(id, 0, "FIND_STOPS");
            status = get_unsigned_value(id, 0, &us8_buf);
            if (us8_buf == FINDSTOPS_SETOPEN || us8_buf == FINDSTOPS_RUNNING )
            {
                delayfor( 3, "|en|\f The valve is moving(%[d]{chk_cnt})...", dummy, dummy, 0);
            }
            ++chk_cnt;
            ACCELERATE_FOR_DEV(chk_cnt += 10);
            if ( chk_cnt > 120 ) {        /*  5sec * 120 = 10min  */
                chk_cnt = 0;
                status = select_from_menu("|en|Continue?\n", dummy, dummy, 0, "Yes;No(Abort)", &ivalue);
                display_message("|en|\n", dummy, dummy, 0);
                if ( ivalue != 1 ) {
                    id = ITEM_ID(PARAM.FIND_STOPS);
                    status = put_unsigned_value(id, 0, FINDSTOPS_CANCEL);
                    SEND_PARAM(id, 0, "FIND_STOPS");
                    METHOD_ABORT_MACRO;
               }
            }
...
METHOD  do_full_close
{
    LABEL "|en|Full Close";
    HELP HLP5("|en|Full Close");
    CLASS OUTPUT;
    DEFINITION
    {
        COMMON_LOCAL_VARS;
        ...
        /* start full close */
        id = ITEM_ID(PARAM.FIND_STOPS);
        status = put_unsigned_value(id, 0, FINDSTOPS_SETCLOSED);
        SEND_PARAM(id, 0, "FIND_STOPS");
        ...
        do {
            ...
            READ_PARAM(id, 0, "FIND_STOPS");
            status = get_unsigned_value(id, 0, &us8_buf);
            if (us8_buf == FINDSTOPS_SETCLOSED || us8_buf == FINDSTOPS_RUNNING )
            {
                delayfor( 3, "|en|The valve is moving(%[d]{chk_cnt})...", dummy, dummy, 0);
            }
            ++chk_cnt;
            ACCELERATE_FOR_DEV(chk_cnt += 10);
            if ( chk_cnt > 120 ) {        /*  5sec * 120 = 10min  */
...
METHOD  do_set_position
{
    LABEL "|en|Set Position";
    HELP HLP5("|en|Set Position");
    CLASS OUTPUT;
    DEFINITION
    {
        COMMON_LOCAL_VARS;
...
METHOD  do_setup_wizard
{
    LABEL "|en|Setup Wizard";
    HELP HLP5("|en|Setup Wizard");
    CLASS OUTPUT;
    DEFINITION
    {
        COMMON_LOCAL_VARS;
        ...
        CHECK_DEVICE_IN_PROCESS;
#if (0)
        get_acknowledgement("1. Should ask to put the block in MAN mode if necesary", ids, dummy, 0);
        get_acknowledgement("2. Quick Start setup", ids, dummy, 0);
        get_acknowledgement("3. Download of the Quick Start parameters", ids, dummy, 0);
        Get_Acknowledgement("4. Auto Find Stops", Ids, Dummy, 0);
        get_acknowledgement("5. Auto Tune(if AT is selected during quick Start)", ids, dummy, 0);
        get_acknowledgement("6. Presentation of the results", ids, dummy, 0);
#endif        
        get_acknowledgement("will be implemented\n", ids, dummy, 0);
...
METHOD  do_factory_configure
{
    LABEL "|en|Factory Reset";
    HELP HLP5("|en|Factory Reset");
    CLASS OUTPUT;
    DEFINITION
    {
        unsigned long   id;
...
METHOD  do_configure_wizard
{
    LABEL "|en|Configure Wizard";
    HELP HLP5("|en|Configure Wizard");
    CLASS OUTPUT;
    DEFINITION
    {
        unsigned long   id;
...
METHOD  display_internal_versions
{
    LABEL "|en|DD time stamps";
    HELP HLP5("|en|display the time stamps of DD source file, used by development purpose");
    CLASS OUTPUT;
    DEFINITION
    {
        unsigned long   id;
        ...
        get_acknowledgement("|en|svi_positioner.ddl:\n"   \
                            TS_SVI_POS                    \
                            "\n\nsvi_faults.h:\n"         \
                            TS_FAULTS_H                   \
                            "\n\nsvi_tb.h:\n"             \
...
METHOD  update_custom_char_points
{
    LABEL "|en|Custom Char. Points";
    HELP "|en|Update the custom characterization points";
    CLASS INPUT;
    DEFINITION
    {
        COMMON_LOCAL_VARS;

		float         char_array_x[NBRS_CHAR_ARRAY / 2], char_array_y[NBRS_CHAR_ARRAY / 2 ];
		int           i,j,p, page;
        int           ivalue;
        int           sel;
        unsigned long ulValue;
        float         fX, fY, fSlope, fSum;

        float fX0, fX1, fX2, fX3, fX4, fX5, fX6, fX7, fX8, fX9, fX10;
        float fX11, fX12,fX13, fX14, fX15, fX16, fX17, fX18;
        float fY0, fY1, fY2, fY3, fY4, fY5, fY6, fY7, fY8, fY9, fY10;
        float fY11, fY12,fY13, fY14, fY15, fY16, fY17, fY18;

        float fIX0, fIY0, fIX1, fIY1, fIX2, fIY2;
        
        int p0, p1, p2, p3, p4, p5, p6;
        int nValue;

        id = ITEM_ID(PARAM.CUSTOM_CHAR_POINTS);
		mb = 0;

		for(i = 0; i < NBRS_CHAR_ARRAY/2 ; i++ )
        {
            mb = i * 2 + 1;
			READ_PARAM(id, mb, "CUSTOM_CHAR_POINTS");
            status=get_signed_value(id, mb, &nValue);
            fX =  nValue * 100.0 / 16384.0;
            char_array_x[i] = fX;

            mb = i * 2 + 2;
            READ_PARAM(id, mb, "CUSTOM_CHAR_POINTS");
            status=get_signed_value(id, mb, &nValue);
            fY =  nValue * 100.0 / 16384.0;
            char_array_y[i] = fY;

            /* p = i + 1; */
            /* display_message("|en|%[i]{p}. (%[5.2f]{fX}, %[5.2f]{fY})\n", dummy, dummy, 0); */
        }

        do {
            fX0  = char_array_x[0]; fY0 = char_array_y[0];
            fX1  = char_array_x[1]; fY1 = char_array_y[1];
            fX2  = char_array_x[2]; fY2 = char_array_y[2];
            fX3  = char_array_x[3]; fY3 = char_array_y[3];
            fX4  = char_array_x[4]; fY4 = char_array_y[4];
            fX5  = char_array_x[5]; fY5 = char_array_y[5];
            fX6  = char_array_x[6]; fY6 = char_array_y[6];
            fX7  = char_array_x[7]; fY7 = char_array_y[7];
            fX8  = char_array_x[8]; fY8 = char_array_y[8];
            fX9  = char_array_x[9]; fY9 = char_array_y[9];
            fX10 = char_array_x[10]; fY10 = char_array_y[10];
            fX11 = char_array_x[11]; fY11 = char_array_y[11];
            fX12 = char_array_x[12]; fY12 = char_array_y[12];
            fX13 = char_array_x[13]; fY13 = char_array_y[13];
            fX14 = char_array_x[14]; fY14 = char_array_y[14];
            fX15 = char_array_x[15]; fY15 = char_array_y[15];
            fX16 = char_array_x[16]; fY16 = char_array_y[16];
            fX17 = char_array_x[17]; fY17 = char_array_y[17];
            fX18 = char_array_x[18]; fY18 = char_array_y[18];


            status = select_from_menu("|en|Select the function in the menu:\n"
                                      "The char. points(x-y pairs):\n" \
                                      "  1. (%[5.2f]{fX0}, %[5.2f]{fY0})\n" \
                                      "  2. (%[5.2f]{fX1}, %[5.2f]{fY1})\n" \
                                      "  3. (%[5.2f]{fX2}, %[5.2f]{fY2})\n" \
                                      "  4. (%[5.2f]{fX3}, %[5.2f]{fY3})\n" \
                                      ...
                                      "  8. (%[5.2f]{fX7}, %[5.2f]{fY7})\n" \
                                      "  9. (%[5.2f]{fX8}, %[5.2f]{fY8})\n" \
                                      " 10. (%[5.2f]{fX9}, %[5.2f]{fY9})\n" \
                                      " 11. (%[5.2f]{fX10}, %[5.2f]{fY10})\n" \
                                      " 12. (%[5.2f]{fX11}, %[5.2f]{fY11})\n" \
                                      " 13. (%[5.2f]{fX12}, %[5.2f]{fY12})\n" \
                                      " 14. (%[5.2f]{fX13}, %[5.2f]{fY13})\n" \
                                      " 15. (%[5.2f]{fX14}, %[5.2f]{fY14})\n" \
                                      " 16. (%[5.2f]{fX15}, %[5.2f]{fY15})\n" \
                                      " 17. (%[5.2f]{fX16}, %[5.2f]{fY16})\n" \
                                      " 18. (%[5.2f]{fX17}, %[5.2f]{fY17})\n" \
                                      " 19. (%[5.2f]{fX18}, %[5.2f]{fY18})\n" \
                                      , dummy, dummy, 0, \
                                      "|en|Load Initial Data;"                    \
                                      "Modify;"                                   \
                                      "Validate;"                                 \
                                      "Save;"                                     \
                                      "Quit\n", &ivalue);
            ...
            case 1:
                ...
                status = edit_device_value("|en|Please select custom char. action:\n",ids, indices, 0, \
                                           ITEM_ID(PARAM.CUSTOM_CHAR), MEMBER_ID(ACTION));
                status = select_from_menu("|en|Do you want to change current char. data?\n", dummy, dummy, 0, \
                                          "Yes;No\n", &ivalue);
                if ( 1 == ivalue )
                {
                    SEND_PARAM( ITEM_ID(PARAM.CUSTOM_CHAR), MEMBER_ID(ACTION), "CUSTOM_CHAR.ACTION");
                    for(i = 0; i < NBRS_CHAR_ARRAY/2 ; i++ )
                    {
                        mb = i * 2 + 1;
                        READ_PARAM(id, mb, "CUSTOM_CHAR_POINTS");
            ...
            case 2:
                ...
                do {
                    ...
                    fX18 = char_array_x[18]; fY18 = char_array_y[18];
                    p = 2;
                    edit_local_value("|en|Enter the nth point(0 - 19) to Edit (0 if Done)" \
                                     "The char. points(x-y pairs):\n"   \
                                     "  1. (%[5.2f]{fX0}, %[5.2f]{fY0})\n" \
                                     "  2. (%[5.2f]{fX1}, %[5.2f]{fY1})\n" \
                                     "  3. (%[5.2f]{fX2}, %[5.2f]{fY2})\n" \
                    ...
                    {
                        if ( p==0) break;
                        get_acknowledgement("Invalid selection", dummy, dummy, 0);
                    } else
                    {
                        i = (p <= 2 ? 1 : p >=18 ? 18 - 1 : p - 1);
                        p0 = i; fIX0 = char_array_x[i-1]; fIY0 = char_array_y[i-1];
                        p1 = i+1; fIX1 = char_array_x[i]; fIY1 = char_array_y[i];
                        p2 = i+2; fIX2 = char_array_x[i+1]; fIY2 = char_array_y[i+1];

                        edit_local_value("|en|Please input the X value for %[i]{p}th. point:\n" \
                                         "  %[i]{p0}. (%[5.2f]{fIX0}, %[5.2f]{fIY0})\n" \
                                         "  %[i]{p1}. (%[5.2f]{fIX1}, %[5.2f]{fIY1})\n" \
                                         "  %[i]{p2}. (%[5.2f]{fIX2}, %[5.2f]{fIY2})\n", dummy, dummy, 0, \
                                         p==1 ? "fIX0" : p==NBRS_CHAR_ARRAY / 2? "fIX2" : "fIX1" );

                        edit_local_value("|en|Please input the Y value for %[i]{p}th. point:\n" \
                                         "  %[i]{p0}. (%[5.2f]{fIX0}, %[5.2f]{fIY0})\n" \
                                         "  %[i]{p1}. (%[5.2f]{fIX1}, %[5.2f]{fIY1})\n" \
                                         "  %[i]{p2}. (%[5.2f]{fIX2}, %[5.2f]{fIY2})\n", dummy, dummy, 0, \
                                         p==1 ? "fIY0" : p==NBRS_CHAR_ARRAY /2? "fIY2" : "fIY1" );
                
                        char_array_x[p-1] = ( p==1 ? fIX0 : p==NBRS_CHAR_ARRAY /2 ? fIX2 : fIX1);
                
                        char_array_y[p-1] = ( p==1 ? fIY0 : p==NBRS_CHAR_ARRAY /2 ? fIY2 : fIY1);
                    }

                } while (1);
                break;
            case 3:
                /* Validate the char. data */

                /*  1. sort the x,y pairs */
                for(i = 0; i < NBRS_CHAR_ARRAY/2 ; i++ )
                {
                    p = i;
                    for (j = i; j < NBRS_CHAR_ARRAY/2 ; j++ )
                    {
                        if (char_array_x[j] < char_array_x[p] ) p = j;
                        /* display_message("|en|%[i]{p}. (%[5.2f]{fX}, %[5.2f]{fY})\n", dummy, dummy, 0); */
                    }
                    fX = char_array_x[p];
                    fY = char_array_y[p];
                    char_array_x[p] = char_array_x[i];
                    char_array_y[p] = char_array_y[i];
                    char_array_x[i] = fX;
                    char_array_y[i] = fY;
                }
                /*  2. validate the conditions */

                /*The slope of the curve should be limited between 2.5 and 87.5
                  degrees. (slope between 0.05 and 20) for up to selected
                  CUSTOM_CHAR.NUMBER_OF_POINTS. If this condition is not true, the
                  curve can be saved but cannot be activated (by writing
                  CHAR_SELECTION.TYPE=255) The initial value is expected to be (if
                  Camflex is selected): */

                fSum = 0.0;
                fSlope = 0.0;

                for( i=0, p = 1; p <= NBRS_CHAR_ARRAY/2 ; p++ )
                {
                    if ( p < 2 || (char_array_x[p-1] - char_array_x[p-2]) < 0.00001 )
                    {
                        fSlope = 0.0;
                    }
                    else
                    {
                        fSlope  = char_array_y[p-1] - char_array_y[p-2];
                        fSlope /= (char_array_x[p-1] - char_array_x[p-2]);
                    }

                    /* display_message("|en|The %[i]{p}slope (%[f]{fSlope})", dummy, dummy, 0); */
                    if ( (fSlope > 0.00001 && fSlope < 0.05) || fSlope > 20.0)
                    {
                        get_acknowledgement("|en|The %[i]{p}th point is out of range[0.05, 20.0]", dummy, dummy, 0);
                        break;
                    }
                    fSum +=fSlope;
                    if (fSum > 87.5)
                    {
                        get_acknowledgement("|en|Total slopes (%[f]{fSum})> 87.5", dummy, dummy, 0);
                        break;
                    }
                }
                if ( fSum < 2.5)
                {
                    get_acknowledgement("|en|Total slopes(%[f]{fSum}) < 2.50 )", dummy, dummy, 0);
                };
                break;
            case 4:
                /* Save to the device */
                ...
                for(i = 0; i < NBRS_CHAR_ARRAY/2 ; i++ )
                {
                    /* following need compiling with -w 718 */
                    nValue = ( char_array_x[i] * 16384 / 100 + 0.5);
                    put_signed_value(id, i*2+1, nValue);
                    SEND_PARAM(id, i*2+1, "CUSTOM_CHAR_POINTS");

                    nValue = ( char_array_y[i] * 16384 / 100 + 0.5);
                    put_signed_value(id, i*2+2, nValue);
                    SEND_PARAM(id, i*2+2, "CUSTOM_CHAR_POINTS");
                ...
                for(i = 0; i < NBRS_CHAR_ARRAY/2 ; i++ )
                {
                    READ_PARAM(id, i * 2 + 1, "CUSTOM_CHAR_POINTS");
                    READ_PARAM(id, i * 2 + 2, "CUSTOM_CHAR_POINTS");
                }
                break;
            case 5:
                /* Quit this method */
                break;
            case 6:
                /* Review the char. data */
                for (p = 0,i = 0; i < NBRS_CHAR_ARRAY / 12; i++)
                {
                    
                    p = i *  NBRS_CHAR_ARRAY /( 2 * 3 );
                    page = i+1;
                    
                    p0 = p+1; fX0 = char_array_x[p+0]; fY0 = char_array_y[p+0];
                    p1 = p+2; fX1 = char_array_x[p+1]; fY1 = char_array_y[p+1];
                    p2 = p+3; fX2 = char_array_x[p+2]; fY2 = char_array_y[p+2];
                    p3 = p+4; fX3 = char_array_x[p+3]; fY3 = char_array_y[p+3];
                    p4 = p+5; fX4 = char_array_x[p+4]; fY4 = char_array_y[p+4];
                    p5 = p+6; fX5 = char_array_x[p+5]; fY5 = char_array_y[p+5];
                    if (page == NBRS_CHAR_ARRAY / 12 )
                    {
                        p6 = p+7; fX6 = char_array_x[p+6]; fY6 = char_array_y[p+6];

                    }
                    
                    get_acknowledgement( page < NBRS_CHAR_ARRAY / 12 ? \
                                         "|en|The char. points(x-y pairs): %[i]{page}/3 Pages\n" \
                                         "  %[i]{p0}. (%[5.2f]{fX0}, %[5.2f]{fY0})\n" \
                                         "  %[i]{p1}. (%[5.2f]{fX1}, %[5.2f]{fY1})\n" \
                                         "  %[i]{p2}. (%[5.2f]{fX2}, %[5.2f]{fY2})\n" \
                                         "  %[i]{p3}. (%[5.2f]{fX3}, %[5.2f]{fY3})\n" \
                                         "  %[i]{p4}. (%[5.2f]{fX4}, %[5.2f]{fY4})\n" \
                                         "  %[i]{p5}. (%[5.2f]{fX5}, %[5.2f]{fY5})\n" \
                                         : \
                                         "|en|The char. points(x-y pairs): %[i]{page}/3 Pages\n" \
                                         "  %[i]{p0}. (%[5.2f]{fX0}, %[5.2f]{fY0})\n" \
                                         "  %[i]{p1}. (%[5.2f]{fX1}, %[5.2f]{fY1})\n" \
