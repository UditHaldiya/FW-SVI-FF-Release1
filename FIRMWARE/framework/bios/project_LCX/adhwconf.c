...
const adhwconf_t adhwconftab =
{
    ...
    .PwrDownMask = MUX_POWER_DOWN,
    .PwrBitsMask = POWER_BIT_MASK,
};
...
{
    GPIO0->IOCLR = adhwconftab.PwrDownMask;       // MUX_POWER_DOWN ;
    GPIO0->IOSET = adhwconftab.PwrBitsMask;       // POWER_BIT_MASK; /* switch all power supplies to OFF */
}
...
void ad_PrepareChannelPower(u8_least channel)
{
    ...
    GPIO0->IOCLR = (adhwconftab.PwrDownMask | (adhwconftab.MuxPwrTable[channel] & adhwconftab.PwrBitsMask));
    GPIO0->IOSET = (adhwconftab.MuxPwrTable[channel] ^ adhwconftab.PwrBitsMask);
...
bool_t  ad_IsChannelPowered(u8_least channel)
{
    bool_t  retval = true;

    if ((adhwconftab.MuxPwrTable[channel] & adhwconftab.PwrBitsMask) == 0u)
    {
